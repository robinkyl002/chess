...
public abstract class MovesCalculator implements MovementRule {
    protected void calculateMoves(ChessBoard board, ChessPosition pos, int rowIncrement, int colIncrement, Collection<ChessMove> moves, boolean allowDistance) {
        if (pos.getRow() + rowIncrement > 8 || pos.getRow() + rowIncrement < 1 || pos.getColumn() + colIncrement > 8 || pos.getColumn() + colIncrement < 1) {
            return;
        }
        ...
        if (board.getPiece(newPos) == null) {
            ...
            if (allowDistance) {
                calculateMoves(board, pos, rowIncrement + Integer.signum(rowIncrement), colIncrement + Integer.signum(colIncrement), moves, allowDistance);
            }
        }
        else {
